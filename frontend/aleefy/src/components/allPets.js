...
function ListPets() {
  ...
  useEffect(() => {
    async function fetchBookings() {
      try {
        const response = await fetch("http://localhost:7000/getAllPets");
        const data = await response.json();
        setBookings(data);
        const uniqueOwners = [...new Set(data.map((pet) => pet.owner))];
  ...
  const handleNoteSubmit = (id) => {
    ...
    if (note) {
      ...
      userService.addNote(send);
      alert(`Note for pet ${id}: ${note}`);
      // Add code to handle the note submission, e.g., save it to the database
    } else {
  ...
  const filteredBookings = selectedOwner
    ? bookings.filter((booking) => booking.owner === selectedOwner)
    : bookings;
            ...
            <div className="col-lg-4" key={index}>
              <div className="card">
                <div className="card-body">
                  ...
                  <h5 className="card-text">Owner: {booking.owner}</h5>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleNoteChange(booking._id, e.target.value)
                    }
                  />
                  <button
                    className="btn btn-lg btn-primary mt-3 px-4"
                    onClick={() => handleNoteSubmit(booking._id)}
                  >
                    Add
                  </button>
                </div>
